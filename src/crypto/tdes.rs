...
pub fn mac_3des(key: &[u8; 16], msg: &[u8]) -> [u8; 8] {
    ...
    for block in msg.chunks_exact(8) {
        for i in 0..8 {
            state[i] ^= block[i];
        }
...
#[cfg(test)]
mod tests {
    use {super::*, crate::crypto::pad, hex_literal::hex};
    ...
    fn test_mac_3des() {
        ...
        let key = hex!("7962D9ECE03D1ACD4C76089DCE131543");
        let mut msg =
            hex!("46B9342A41396CD7386BF5803104D7CEDC122B9132139BAF2EEDC94EE178534F").to_vec();
        let mac = hex!("2F2D235D074D7449");
        pad(&mut msg, 8);
        assert_eq!(mac_3des(&key, &msg), mac);

        let key = hex!("F1CB1F1FB5ADF208806B89DC579DC1F8");
        let mut msg = hex!("887022120C06C2270CA4020C800000008709016375432908C044F6").to_vec();
